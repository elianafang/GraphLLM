...
class MiragePlusRuntime:
    ...
    def __init__(self, script_name: str, log_dir: Path, config: Any):
        ...
        self.graph_schema: Dict[str, Any] = {
            "node_types": sorted(self.allowed_node_types),
            "edge_types": sorted(self.allowed_edge_types),
            "node_properties": [
                "confidence",
                "source_agent",
                "source_round",
        ...
        initial_snapshot = self._build_graph_snapshot_data(round_index=0)
        self.graph_state = initial_snapshot
        self._ensure_file(self.graph_path, initial_snapshot)
    ...
    def _write_graph_snapshot(self, round_index: int) -> None:
        snapshot = self._build_graph_snapshot_data(round_index)
    ...
    def _build_graph_snapshot_data(self, round_index: int) -> Dict[str, Any]:
        timestamp = datetime.utcnow().isoformat() + "Z"
        return {
            "script": self.script_name,
            "round": round_index,
